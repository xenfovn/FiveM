        ...
        if QBCore ~= nil then
            ...
            if sellBackDist <= 13.0 and IsPedInAnyVehicle(ped) then 
                ...
                if sellBackDist <= 3.5 and IsPedInAnyVehicle(ped) then
                    ...
                    DrawText3Ds(Config.SellVehicleBack.x, Config.SellVehicleBack.y, Config.SellVehicleBack.z, '[~g~E~w~] - Bán xe cho đại lý ~g~$'..math.floor(sellVehData.price / 2))
                    if IsControlJustPressed(0, 38) then
                        QBCore.Functions.TriggerCallback('qb-garage:server:checkVehicleOwner', function(owned)
                            if owned then
                                ...
                                QBCore.Functions.Notify('Đây không phải là chiếc xe của bạn..', 'error', 3500)
                            end
            ...
            if inRange then
                    ...
                    if dstCheck <= 2 then
                        if not IsPedInAnyVehicle(ped) then
                            if not isConfirming then
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.45, '[~g~E~w~] - Xem hợp đồng xe')
                                ...
                                if Config.OccasionSlots[i]["owner"] == QBCore.Functions.GetPlayerData().citizenid then
                                    DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.05, '[~r~G~w~] - Hủy bỏ bán xe')
                                ...
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.45, 'Bạn có chắc chắn bạn không còn muốn bán xe của bạn nữa?')
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.25, '[~g~7~w~] - Mua | [~r~8~w~] - Không')
                ...
                if sellDist <= 13.0 and IsPedInAnyVehicle(ped) then 
                    ...
                    if sellDist <= 3.5 and IsPedInAnyVehicle(ped) then
                        DrawText3Ds(Config.SellVehicle.x, Config.SellVehicle.y, Config.SellVehicle.z, '[~g~E~w~] -Đặt xe để bán bởi chủ sở hữu')
                        if IsControlJustPressed(0, 38) then
...
AddEventHandler('qb-occasions:client:BuyFinished', function(vehdata)
    ...
    QBCore.Functions.SpawnVehicle(vehdata.model, function(veh)
        ...
        QBCore.Functions.Notify("Xe mua", "success", 2500)
...
AddEventHandler('qb-occasions:client:ReturnOwnedVehicle', function(vehdata)
    ...
    QBCore.Functions.SpawnVehicle(vehdata.model, function(veh)
        ...
        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
        SetVehicleFuelLevel(veh, 100)
        QBCore.Functions.Notify("Xe của bạn được trả lại")
        TriggerEvent("vehiclekeys:client:SetOwner", vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Citizen.Wait(500)
        QBCore.Functions.SetVehicleProperties(veh, vehmods)
...
function sellVehicleWait(price)
    ...
    Citizen.Wait(1500)
    DoScreenFadeIn(250)
    QBCore.Functions.Notify('Xe của bạn đã được đưa lên để bán! Giá bán : $'..price, 'success')
...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Mua/Bán Xe Cũ")
