...
function PullOutVehicle()
    if ownsVan == true then
        QBCore.Functions.Notify("Bạn đã có xe , tiếp tục hoặc dừng công việc của bạn", "error")
    ...
    while true do
        ...
        if Hired then
            ...
            if dist <= 2.5 then
                ...
                DrawText3Ds(GaragePos["x"],GaragePos["y"],GaragePos["z"], "Nhấn [~g~G~s~] để bất đầu công việc")
                if dist <= 3.0 then
...
function DrawTarget()
    ...
    if DeliveriesCount == 4 then
        QBCore.Functions.Notify("Tất cả pizza đã giao xong","success")
        RemoveCancelBlip()
      ...
      if pizza == 1 then
        QBCore.Functions.Notify("Bạn đã có 1 hộp pizza","success")
      else
        if pizza == 4 then
          pizza = 'Four'
        elseif pizza == 3 then
        ...
        QBCore.Functions.Notify("Bạn Có "..pizza.." pizzas","success")
      end
        if LastGoal == RandomPoint then
		    ...
		    QBCore.Functions.Notify("Đi tới điểm giao hàng","success")
        end
    end
end
...
Citizen.CreateThread(function()
    while true do
        ...
        if dist <= 20.0 and Hired and (not HasPizza) then
            ...
            DrawText3Ds(DeliveryPoint["x"],DeliveryPoint["y"],DeliveryPoint["z"], "Lấy ~y~Pizza~s~ Từ xe ~b~PANTO~s~!")
            ...
            if IsVehicleModel(Vehicle, GetHashKey('panto')) then
                ...
                DrawText3Ds(VehPos.x,VehPos.y,VehPos.z, "Nhấn [~g~G~s~] để lấy  ~y~Pizza")
        ...
        elseif dist <= 25 and HasPizza and Hired then
            ...
            DrawText3Ds(DeliveryPoint["x"],DeliveryPoint["y"],DeliveryPoint["z"], "Nhấn [~g~G~s~] để đưa ~y~Pizza")
            if dist <= 3 then
                if IsControlJustReleased(0, 47) then
                    TakePizza()
                    DeliverPizza()
            ...
            if Delivered then
                if DistanceFromEndZone <= 2.5 then
                    ...
                    if DistanceFromEndZone <= 7 then
                        if IsControlJustReleased(0, 47) then
                            QBCore.Functions.Notify("Tất cả đã xong , hãy tới điểm tiếp theo", "success")
                            EndOfWork()
                        end
                    end
                end
                ...
                if DistanceFromCancelZone <= 2.5 then
                    ...
                    DrawText3Ds(cancel["x"],cancel["y"],cancel["z"], "Nhấn [~g~G~s~] để ~r~hủy~s~ pizza")
...
function EndOfWork()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        if IsVehicleModel(Van, GetHashKey('panto')) then
            ...
            QBCore.Functions.Notify("Bạn cần phải vào xe công ty", "error")
            QBCore.Functions.Notify("Mất xe công ty , mất việc", "error")
        end
    else
        InService = false
        BlipSell = nil
