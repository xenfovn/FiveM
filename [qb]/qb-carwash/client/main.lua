                    ...
                    if dist <= 7.5 then
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                DrawText3Ds(Config.Locations[k]["coords"]["x"], Config.Locations[k]["coords"]["y"], Config.Locations[k]["coords"]["z"], '~g~E~w~ - Rửa xe ô tô($'..Config.DefaultPrice..')')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerServerEvent('qb-carwash:server:washCar')
                                end
                            else
                                DrawText3Ds(Config.Locations[k]["coords"]["x"], Config.Locations[k]["coords"]["y"], Config.Locations[k]["coords"]["z"], 'Rửa xe không có sẵn ..')
                            end
                        end
                    end
                end
...
AddEventHandler('qb-carwash:client:washCar', function()
    ...
    QBCore.Functions.Progressbar("search_cabin", "Xe đang được rửa ..", math.random(4000, 8000), false, true, {
        ...
        QBCore.Functions.Notify("Rửa bị hủy bỏ ..", "error")
        washingVehicle = false
    end)
end)
