	...
	if Patient ~= nil then
		if oldMan then
			if Player.Functions.RemoveMoney("cash", 5000, "revived-player") then
				...
				TriggerClientEvent('QBCore:Notify', src, "Bạn không có đủ tiền cho bạn ..", "error")
			end
		else
			Player.Functions.RemoveItem('firstaid', 1)
			TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['firstaid'], "remove")
...
AddEventHandler('hospital:server:SendDoctorAlert', function()
	...
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		if Player ~= nil then 
			if (Player.PlayerData.job.name =="ambulance" and Player.PlayerData.job.onduty) then
				TriggerClientEvent("hospital:client:SendAlert", v, "Cần Một Bác Sĩ ở Trong Bệnh Viện")
			end
		end
	end
end)
...
AddEventHandler('hospital:server:CanHelp', function(helperId, canHelp)
	...
	if canHelp then
		...
		TriggerClientEvent('QBCore:Notify', helperId, "Bạn không thể giúp người này ..", "error")
	end
end)