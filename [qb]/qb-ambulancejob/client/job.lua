    ...
    while true do
        ...
        if isLoggedIn and QBCore ~= nil then
            ...
            if PlayerJob.name =="ambulance" or PlayerJob.name == "ambulance" then
                ...
                for k, v in pairs(Config.Locations["armory"]) do
                    ...
                    if dist < 4.5 then
                        if onDuty then
                            if dist < 1.5 then
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Mở Tủ Đồ")
                                if IsControlJustReleased(0, 38) then
                    ...
                    if dist < 4.5 then
                        ...
                        if dist < 1.5 then
                            if IsPedInAnyVehicle(ped, false) then
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Cửa hàng xe")
                            else
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Xe")
                            end
                            if IsControlJustReleased(0, 38) then
                                if IsPedInAnyVehicle(ped, false) then
                ...
                for k, v in pairs(Config.Locations["helicopter"]) do
                    ...
                    if dist < 7.5 then
                        if onDuty then
                            ...
                            if dist < 1.5 then
                                if IsPedInAnyVehicle(ped, false) then
                                    DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Cửa Hàng Trực Thăng")
                                else
            ...
            for k, v in pairs(Config.Locations["main"]) do
                local dist = #(pos - vector3(v.x, v.y, v.z))
                if dist < 1.5 then
                    DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Đi thang máy đến mái nhà")
                    if IsControlJustReleased(0, 38) then
                        DoScreenFadeOut(500)
            ...
            for k, v in pairs(Config.Locations["roof"]) do
                local dist = #(pos - vector3(v.x, v.y, v.z))
                if dist < 1.5 then
                    DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Đi thang máy xuống")
                    if IsControlJustReleased(0, 38) then
                        DoScreenFadeOut(500)
                        while not IsScreenFadedOut() do
                            Citizen.Wait(10)
                ...
                for k, v in pairs(result) do
                    ...
                    elseif result["WEAPONWOUNDS"] ~= nil then
                        for k, v in pairs(result["WEAPONWOUNDS"]) do
                            TriggerEvent("chatMessage", "KIỂM TRA TÌNH TRẠNG", "error", WeaponDamageList[v])
                        end
                    elseif result["BLEED"] > 0 then
                        TriggerEvent("chatMessage", "KIỂM TRA TÌNH TRẠNG", "error", "Is "..Config.BleedingStates[v].label)
                    else
                        QBCore.Functions.Notify('Người chơi khỏe mạnh', 'success')
                    end
                end
    ...
    QBCore.Functions.TriggerCallback('hospital:server:HasFirstAid', function(hasItem)
        if hasItem then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                QBCore.Functions.Progressbar("hospital_revive", "Đang Tiến Hành Cứu..", 5000, false, true, {
                    ...
                    QBCore.Functions.Notify("Bạn đã cứu người đó!")
            ...
            QBCore.Functions.Notify("Bạn cần một bộ sơ cứu", "error")
        end
    end, 'firstaid')
end)
    ...
    QBCore.Functions.TriggerCallback('hospital:server:HasBandage', function(hasItem)
        if hasItem then
            ...
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                isHealingPerson = true
                QBCore.Functions.Progressbar("hospital_healwounds", "Chữa lành vết thương..", 5000, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                    ...
                    isHealingPerson = false
                    StopAnimTask(PlayerPedId(), healAnimDict, "exit", 1.0)
                    QBCore.Functions.Notify("Bạn đã giúp người đó!")
            ...
            QBCore.Functions.Notify("Bạn cần một băng gạc", "error")
        end
    end, 'bandage')
end)
...
function VehicleList(isDown)
    ...
    for k, v in pairs(Config.Vehicles) do
        Menu.addButton(Config.Vehicles[k], "TakeOutVehicle", {k, isDown}, "Garage", " Động cơ: 100%", " Thân hình: 100%", " Nhiên Liệu: 100%")
    end
