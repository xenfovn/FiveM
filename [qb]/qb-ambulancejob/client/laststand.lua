        ...
        Citizen.CreateThread(function()
            while InLaststand do
                ...
                elseif LaststandTime - 1 <= 0 then
                    QBCore.Functions.Notify("Bạn đã nổ tung..", "error")
                    SetLaststand(false)
...
AddEventHandler('hospital:client:UseFirstAid', function()
    if not isEscorting then
        ...
        QBCore.Functions.Notify('Hành động không thể!', 'error')
    end
end)
...
AddEventHandler('hospital:client:HelpPerson', function(targetId)
    ...
    QBCore.Functions.Progressbar("hospital_revive", "Cứu người..", math.random(30000, 60000), false, true, {
        ...
        ClearPedTasks(ped)
        QBCore.Functions.Notify("Bạn đã cứu một người.")
