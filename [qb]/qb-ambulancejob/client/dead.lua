	...
	while true do
	    ...
	    EnableControlAction(0, 249, true)
            ...
            if isDead then
                if not isInHospitalBed then 
                    if deathTime > 0 then
                        DrawTxt(0.93, 1.44, 1.0,1.0,0.6, "Hồi sinh trong: ~r~" .. math.ceil(deathTime) .. "~w~ Giây", 255, 255, 255, 255)
                    else
                        DrawTxt(0.865, 1.44, 1.0, 1.0, 0.6, "~w~ GIỮ ~r~[E] ("..hold..")~w~ Để Hồi Sinh(Mất Toàn Bộ Vật Phẩm) ~r~($"..Config.BillCost..")~w~", 255, 255, 255, 255)
                    end
                end
		...
		EnableControlAction(0, 249, true)
                ...
                if LaststandTime > Laststand.MinimumRevive then
                    DrawTxt(0.94, 1.44, 1.0, 1.0, 0.6, "Bạn đang chảy máu trong: ~r~" .. math.ceil(LaststandTime) .. "~w~ Giây", 255, 255, 255, 255)
                else
                    DrawTxt(0.845, 1.44, 1.0, 1.0, 0.6, "Bạn đang chảy máu trong: ~r~" .. math.ceil(LaststandTime) .. "~w~ Giây, Bạn có thể được giúp đỡ", 255, 255, 255, 255)
                end
