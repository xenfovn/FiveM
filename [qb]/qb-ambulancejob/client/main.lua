...
Citizen.CreateThread(function()
    ...
    AddTextComponentString("Bệnh Viện Paleto")
    ...
    SetBlipColour(blip, 25)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString("Bệnh Viện Thành Phố")
    EndTextCommandSetBlipName(blip)
        ...
        if QBCore ~= nil then
            ...
            if #(pos - vector3(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z)) < 1.5 then
                if doctorCount >= Config.MinimalDoctors then
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "~g~E~w~ - Gọi bác sĩ")
                else
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "~g~E~w~ - Đăng Ký Nhập Viện")
                end
                if IsControlJustReleased(0, 38) then
                    if doctorCount >= Config.MinimalDoctors then
                        ...
                        QBCore.Functions.Progressbar("hospital_checkin", "Đăng Ký..", 2000, false, true, {
                            ...
                            if bedId ~= nil then 
                                ...
                                QBCore.Functions.Notify("Giường bị chiếm đóng..", "error")
                            end
                        end, function() -- Cancel
                            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                            QBCore.Functions.Notify("Đăng ký thất bại!", "error")
                        end)
                    end
                end
            elseif #(pos - vector3(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z)) < 4.5 then
                if doctorCount >= Config.MinimalDoctors then
                    ...
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "Đăng ký vào")
                end
            end
            ...
            if closestBed ~= nil and not isInHospitalBed then
                if #(pos - vector3(Config.Locations["beds"][closestBed].x, Config.Locations["beds"][closestBed].y, Config.Locations["beds"][closestBed].z)) < 1.5 then
                    QBCore.Functions.DrawText3D(Config.Locations["beds"][closestBed].x, Config.Locations["beds"][closestBed].y, Config.Locations["beds"][closestBed].z + 0.3, "~g~E~w~ - Nằm trên giường")
                    if IsControlJustReleased(0, 38) then
                        if GetAvailableBed(closestBed) ~= nil then 
                            TriggerServerEvent("hospital:server:SendToBed", closestBed, false)
                        else
                            QBCore.Functions.Notify("Hết giường..", "error")
                        end
                    end
                end
            end
    ...
    while true do
        ...
        if QBCore ~= nil then
            if isInHospitalBed and canLeaveBed then
                local pos = GetEntityCoords(PlayerPedId())
                QBCore.Functions.DrawText3D(pos.x, pos.y, pos.z, "~g~E~w~ - Xuất Viện..")
                if IsControlJustReleased(0, 38) then
                    LeaveBed()
                end
            else
...
AddEventHandler('hospital:client:Revive', function()
    ...
    QBCore.Functions.Notify("Bạn đã bình phục sức khỏe!")
end)
...
function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        QBCore.Functions.Notify("Bạn Đang "..Config.BleedingStates[tonumber(isBleeding)].label, "error", 5000)
    end
end
...
function MenuOutfits()
    MenuTitle = "Trang phục."
    ClearMenu()
    Menu.addButton("Trang phục của tôi", "OutfitsLijst", nil)
    ...
    QBCore.Functions.TriggerCallback('apartments:GetOutfits', function(outfits)
        ...
        if outfits == nil then
            ...
            for k, v in pairs(outfits) do
                Menu.addButton(outfits[k].outfitname, "Tùy chọn", outfits[k]) --optionMenu
            end
        end
        Menu.addButton("Trở lại", "Menu Trang phục",nil) --Back,MenuOutfits
    end)
end

function optionMenu(outfitData)
    MenuTitle = "Gì bây giờ?"
    ClearMenu()

    Menu.addButton("Chọn trang phục", "selectOutfit", outfitData) 
    Menu.addButton("Xóa trang phục.", "removeOutfit", outfitData) 
...
function selectOutfit(oData)
    ...
    QBCore.Functions.Notify(oData.outfitname.." Được chọn", "success", 2500)
    closeMenuFull()
    changeOutfit()
end
