            ...
            if PlayerJob.name == "trucker" then
                ...
                if #(pos - vector3(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z)) < 10.0 then
                    ...
                    if #(pos - vector3(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z)) < 1.5 then
                        if IsPedInAnyVehicle(PlayerPedId(), false) then
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Trả Xe")
                        else
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Lấy Xe")
                        end
                            ...
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                    if isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                                        ...
                                        QBCore.Functions.Notify('Đây không phải là phương tiện thương mại!', 'error')
                                    end
                                else
                                    QBCore.Functions.Notify('Bạn phải là một người lái xe..')
                                end
                            else
                                MenuGarage()
                ...
                if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 4.5 then
                    if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 1.5 then
                        DrawText3D(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, "~g~E~w~ - Thanh Toán Lương")
                        if IsControlJustReleased(0, 38) then
                            if JobsDone > 0 then
                                ...
                                QBCore.Functions.Notify("Giao Hàng Đi mới có tiền lương anh trai.", "error")
                            end
                        end
                            ...
                            if not hasBox then
                                ...
                                if isTruckerVehicle(vehicle) and CurrentPlate == GetVehicleNumberPlateText(vehicle) then
                                    ...
                                    if #(pos - vector3(trunkpos.x, trunkpos.y, trunkpos.z)) < 1.5 and not isWorking then
                                        DrawText3D(trunkpos.x, trunkpos.y, trunkpos.z, "~g~E~w~ - Lấy Hàng")
                                        if IsControlJustReleased(0, 38) then
                                            isWorking = true
                                            QBCore.Functions.Progressbar("work_carrybox", "Mở Xe Lấy Hàng", 2000, false, true, {
                                        ...
                                        DrawText3D(trunkpos.x, trunkpos.y, trunkpos.z, "Lấy Hàng")
                                    end
                                end
                            elseif hasBox then
                                if #(pos - vector3(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z)) < 1.5 and not isWorking then
                                    DrawText3D(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, "~g~E~w~ - Giao Hàng")
                                    if IsControlJustReleased(0, 38) then
                                        isWorking = true
                                        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                                        Citizen.Wait(500)
                                        TriggerEvent('animations:client:EmoteCommandStart', {"bumbin"})
                                        QBCore.Functions.Progressbar("work_dropbox", "Đang Bàn Giao Hàng Hóa", 2000, false, true, {
                                            disableMovement = true,
                                            disableCarMovement = true,
                                            disableMouse = false,
                                            disableCombat = true,
                                            ...
                                            if currentCount == CurrentLocation.dropcount then
                                                ...
                                                QBCore.Functions.Notify("Bạn đã cung cấp tất cả các sản phẩm, đến điểm tiếp theo")
                                    ...
                                    DrawText3D(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, "Cung cấp sản phẩm")
                                end
                            end
                        end
                    end
...
function getNewLocation()
    ...
    if location ~= 0 then
        ...
        QBCore.Functions.Notify("Bạn đã đi đến tất cả các cửa hàng .. Về nhận lương nào!")
        if CurrentBlip ~= nil then
            RemoveBlip(CurrentBlip)
            CurrentBlip = nil
        end
