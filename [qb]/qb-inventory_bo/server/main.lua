...
AddEventHandler('inventory:server:GiveItem', function(name, inventory, item, amount)
	...
	local Target = OtherPlayer.PlayerData.name
	local itemInfo = QBCore.Shared.Items[item.name:lower()]
	local Sajat = Player.PlayerData.name
		if amount <= item.amount then
			if OtherPlayer.Functions.AddItem(itemInfo["name"], amount, false, info) then
				-- OtherPlayer.Functions.AddItem(item.name, amount, false, item.info)
				Player.Functions.RemoveItem(item.name, amount)
				TriggerClientEvent('QBCore:Notify', src, "Átadtál "..Target.." játékosnak "..item.label..":"..amount.."!")
				TriggerClientEvent('QBCore:Notify', name, "Kaptál "..Sajat.." játékostól "..item.label..":"..amount.."!")
			else
				TriggerClientEvent('QBCore:Notify', source,  "Nem fél el az illetőnél több tárgy!", "error")
			end
		else
			TriggerClientEvent('QBCore:Notify', src, "Nincs elegendő tárgyad az átadáshoz")
		end
end)
