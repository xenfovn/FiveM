...
QBCore.Functions.IsPlayerBanned = function (source)
	local retval = false
	local message = ""
	--[[ QBCore.Functions.ExecuteSql(false, "SELECT * FROM `bans` WHERE `steam` = '"..GetPlayerIdentifiers(source)[1].."' OR `license` = '"..GetPlayerIdentifiers(source)[2].."' OR `ip` = '"..GetPlayerIdentifiers(source)[3].."'", function(result)
		if result[1] ~= nil then 
			if os.time() < result[1].expire then
				...
				QBCore.Functions.ExecuteSql(false, "DELETE FROM `bans` WHERE `id` = "..result[1].id)
			end
		end
	end) ]]
