...
local function OnPlayerConnecting(name, setKickReason, deferrals)
    ...
    deferrals.update(string.format("Xin Chào %s. ID Steam của bạn đang được kiểm tra.", name))
    ...
    deferrals.update(string.format("Xin Chào %s. Chúng tôi đang kiểm tra nếu bạn bị cấm.", name))
	
    local isBanned, Reason = QBCore.Functions.IsPlayerBanned(player)
	
    Wait(2500)
	
    deferrals.update(string.format("Chào mừng %s đến {LOS SANTOS CITY}.", name))

    if not steamIdentifier then
        deferrals.done("Bạn không được kết nối với Steam.")
...
AddEventHandler('QBCore:server:CloseServer', function(reason)
    ...
    if QBCore.Functions.HasPermission(source, "admin") or QBCore.Functions.HasPermission(source, "god") then 
        local reason = reason ~= nil and reason or "Không có lý do quy định..."
        QBCore.Config.Server.closed = true
        QBCore.Config.Server.closedReason = reason
        TriggerClientEvent("qbadmin:client:SetServerStatus", -1, true)
	else
		QBCore.Functions.Kick(src, "Bạn không có quyền cho việc này..", nil, nil)
    end
end)
			...
			if Player ~= nil then
				...
				if (QBCore.Functions.HasPermission(source, "god") or QBCore.Functions.HasPermission(source, QBCore.Commands.List[command].permission)) then
					if (QBCore.Commands.List[command].argsrequired and #QBCore.Commands.List[command].arguments ~= 0 and args[#QBCore.Commands.List[command].arguments] == nil) then
					    TriggerClientEvent('QBCore:Notify', source, "Tất cả các đối số phải được điền!", "error")
					    local agus = ""
					...
					TriggerClientEvent('QBCore:Notify', source, "Không có quyền truy cập vào lệnh này", "error")
				end
			end
		end
	end
...
AddEventHandler('QBCore:ToggleDuty', function()
	...
	if Player.PlayerData.job.onduty then
		...
		TriggerClientEvent('QBCore:Notify', src, "Bạn Đang Ngoài Ca Làm Việc!")
	else
		Player.Functions.SetJobDuty(true)
		TriggerClientEvent('QBCore:Notify', src, "Bạn Đang Trong Ca Làm Việc!")
	end
