                        ...
                        if InVehicle then
                            ...
                            if IsControlJustReleased(0, 38) then
                                QBCore.Functions.TriggerCallback('qb-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                        QBCore.Functions.Notify("You have $1000,- deposit returned!")
                                    else
                                ...
                                QBCore.Functions.TriggerCallback('qb-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        ...
                                        QBCore.Functions.SpawnVehicle("trash2", function(veh)
                                            ...
                                            IsWorking = true
                                            SetGarbageRoute()
                                            QBCore.Functions.Notify("Bạn có $ 50, - Tiền gửi được trả!")
                                            QBCore.Functions.Notify("Bạn đã bắt đầu làm việc, vị trí được đánh dấu trên GPS!")
                                        end, coords, true)
                                    else
                                        QBCore.Functions.Notify("Bạn không có đủ tiền để cọc xe .. Chi phí tiền gửi là $ 50,-")
                                    end
                                end)
                            end
                        end
                        ...
                        if DeliveryBlip ~= nil then
                            ...
                            if Distance < 20 or hasZak then
                                ...
                                if not hasZak then
                                    if CanTakeBag then
                                        if Distance < 1.5 then
                                            DrawText3D2(DeliveryData.coords, "~g~E~w~ - Lấy một túi rác")
                                        ...
                                        elseif Distance < 10 then
                                            DrawText3D2(DeliveryData.coords, "Đứng ở đây để lấy một túi rác.")
                                        end
                                    end
                                else
                                    if DoesEntityExist(GarbageVehicle) then
                                        if Distance < 10 then
                                            DrawText3D2(DeliveryData.coords, "Đặt túi vào xe tải của bạn..")
                                        end
                                        ...
                                        if TruckDist < 2 then
                                            DrawText3D(Coords.x, Coords.y, Coords.z, "~g~E~w~ - Vứt bỏ rác vào xe")
                                            if IsControlJustPressed(0, 51) then
                                                hasZak = false
                                                ...
                                                if (AmountOfBags - 1) == 0 then
                                                    ...
                                                    Earnings = Earnings + math.random(27, 35)
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        ...
                                                        SetGarbageRoute()
                                                        QBCore.Functions.Notify("Tất cả các túi rác được thực hiện, tiến tới vị trí tiếp theo!")
                                                    else
                                                        -- Hier ben je klaar met werken.
                                                        QBCore.Functions.Notify("Bạn đã làm việc xong! Quay trở lại kho.")
                                                        IsWorking = false
                                                        RemoveBlip(DeliveryBlip)
                                                        SetRouteBack()
                                                    end
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        QBCore.Functions.Notify("Vẫn còn "..AmountOfBags.." túi!")
                                                    else
                                                        QBCore.Functions.Notify("Ở đó vẫn còn "..AmountOfBags.." túi ở đó!")
                                                    end
                                                    hasZak = false
                                                end
                                                DeliverAnim()
                                        ...
                                        elseif TruckDist < 10 then
                                            DrawText3D(Coords.x, Coords.y, Coords.z, "Đứng đây..")
                                        end
                                    else
                                        DrawText3D2(DeliveryData.coords, "Bạn không có xe tải..")
                                    end
                                end
                            end
                        end
