    ...
    if whitelistedVehicle() then
       -- if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                QBCore.Functions.Notify('Hãy đến địa điểm trên bản đồ, khách hàng đang chờ bạn', 'success')
                    ...
                    while not NpcData.NpcTaken do
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                DrawText3D(Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].x, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].y, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].z, '[E] Mời NPC Lên Xe')
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    QBCore.Functions.Notify('Chở khách đến nơi yêu cầu')
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                QBCore.Functions.Notify('Bạn đã nhận đón 1 khách khác')
            end
      --  else
         --   QBCore.Functions.Notify('No NPCs Are Available')
       -- end
    else
        QBCore.Functions.Notify('Bạn không ở trong một chiếc taxi')
    end
end)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    local npccoords = GetEntityCoords(NpcData.Npc)
                    DrawText3D(Config.NPCLocations.DeliverLocations[NpcData.CurrentDeliver].x, Config.NPCLocations.DeliverLocations[NpcData.CurrentDeliver].y, Config.NPCLocations.DeliverLocations[NpcData.CurrentDeliver].z, '[E] Để Khách Xuống Xe')
                    if IsControlJustPressed(0, 38) then
                        local veh = GetVehiclePedIsIn(ped, 0)
                        ...
                        QBCore.Functions.Notify('Khách đã xuống xe và thanh toán tiền', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
AddEventHandler('qb-taxi:client:toggleMeter', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            QBCore.Functions.Notify('Chiếc xe này không có đồng hồ taxi', 'error')
        end
    else
...
AddEventHandler('qb-taxi:client:enableMeter', function()
    ...
    if meterIsOpen then
        ...
        QBCore.Functions.Notify('Đồng hồ taxi không hoạt động', 'error')
    end
end)
...
AddEventHandler('qb-taxi:client:toggleMuis', function()
    ...
    if meterIsOpen then
        ...
        QBCore.Functions.Notify('Không có máy đo taxi trong tầm nhìn', 'error')
    end
end)
...
function TaxiGarage()
    ...
    MenuTitle = "Garage"
    ClearMenu()
    Menu.addButton("Xe", "VehicleList", nil)
...
function VehicleList()
    ...
    Menu.addButton("Trở Lại", "Nhà Để Xe Taxi",nil)
end
