            ...
            if companyName ~= "" then
                if IsNameAvailable(companyName) then
                    if cashAmount >= Config.CompanyPrice then
                        ...
                        TriggerClientEvent("qb-companies:client:setCompanies", -1, Config.Companies)
                        TriggerClientEvent('QBCore:Notify', src, "Xin chúc mừng, bạn là chủ sở hữu tự hào của: "..companyLabel)
                    else
                ...
                TriggerClientEvent('QBCore:Notify', src, 'Tên không thể trống ..', 'error', 4000)
            end
        else
            TriggerClientEvent('QBCore:Notify', src, 'Tên không thể trống', 'error', 4000)
        end
    else
        TriggerClientEvent('QBCore:Notify', src, 'Bạn không có đủ tiền', 'error', 4000)
    end
end)
...
AddEventHandler('qb-companies:server:removeCompany', function(companyName)
    ...
    if companyName ~= nil then 
        if IsCompanyOwner(companyName, Player.PlayerData.citizenid) then
            ...
            TriggerClientEvent("qb-phone:client:InPhoneNotify", src, "Businesses", "success", "Bạn có " .. companyLabel .. " deleted!")
        else
            TriggerClientEvent('QBCore:Notify', src, 'Bạn không sở hữu doanh nghiệp này ..', 'error', 4000)
        end
    else
        TriggerClientEvent('QBCore:Notify', src, 'Tên không thể trống ..', 'error', 4000)
    end
