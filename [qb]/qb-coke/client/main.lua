...
Citizen.CreateThread(function()
    while true do
        ...
        if distance1 < 15 then
            ...
            if distance1 < 2 then
                DrawText3Ds(509.89, 6478.76, 29.77, "[G] Hái")
                if IsControlJustPressed(0, 47) then
                    PrepareAnim()
                    PickMinigame()
                end
            ...
            if distance2 < 2 then
                DrawText3Ds(509.47, 6472.89, 29.77, "[G] Hái")
                if IsControlJustPressed(0, 47) then
                    PrepareAnim()
                    PickMinigame()
                end
            ...
            if distance3 < 2 then
                DrawText3Ds(509.47, 6467.10, 29.74, "[G] Hái")
                if IsControlJustPressed(0, 47) then
                    PrepareAnim()
                    PickMinigame()
                end
...
Citizen.CreateThread(function()
    while true do
        ...
        if distance < 6 then
            ...
            if distance < 2 then
                DrawText3Ds(-1078.21, -1678.42, 4.57, "[G] Chế Tạo")
                if IsControlJustPressed(0, 47) then
...
function pickProcess()
    QBCore.Functions.Progressbar("grind_coke", "Nhặt lá coca ..", math.random(80000,120000), false, true, {
...
function cokeProcess()
    QBCore.Functions.Progressbar("grind_coke", "Xử lý lá coca ..", math.random(10000, 12000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
function crackProcess()
    QBCore.Functions.Progressbar("grind_coke", "Chế Tạo ..", math.random(10000, 12000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
function PickMinigame()
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            pickProcess()
            QBCore.Functions.Notify("Bạn chọn một chiếc lá coca", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
            ...
            QBCore.Functions.Notify("Bạn làm rối tung lá coke!", "error")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
            cokepicking = false
...
function ProcessMinigame(source)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            cokeProcess()
            QBCore.Functions.Notify("Bạn làm một ít coke!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
            ...
            QBCore.Functions.Notify("Bạn đã gây rối quá trình!", "error")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
            cokeprocess = false
...
function ProcessCrackMinigame(source)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            crackProcess()
            QBCore.Functions.Notify("Bạn làm một số chế tạo!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
...
function knockDoorAnim(home)
    ...
    if home then
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        QBCore.Functions.Notify('Dường như không có ai ở nhà..', 'error', 3500)
    end
end
