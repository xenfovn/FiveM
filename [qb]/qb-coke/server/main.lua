            ...
            if cocaleaf ~= nil then
                if ItemList[Player.PlayerData.items[k].name] ~= nil then 
                    if Player.PlayerData.items[k].name == "cocaleaf" and Player.PlayerData.items[k].amount >= 2 then 
                        ...
                        TriggerClientEvent('QBCore:Notify', src, "Bạn không có đủ lá coke", 'error')
                        break
                    end
                end
            else
                TriggerClientEvent('QBCore:Notify', src, "Bạn không có lá coke", 'error')
                break
            end
        end
    end
        ...
        if Player.PlayerData.items ~= nil then 
            if cokebaggy ~= nil then 
                if cokebaggy.amount >= 2 then 
                    ...
                    TriggerClientEvent('QBCore:Notify', src, "Bạn không có các mặt hàng chính xác", 'error')   
                end
            else
                TriggerClientEvent('QBCore:Notify', src, "Bạn không có các mặt hàng chính xác", 'error')   
            end
        else
            TriggerClientEvent('QBCore:Notify', src, "Bạn chẳng có gì...", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "Bạn có thể cần phải nói chuyện với một thành viên băng đảng...", 'error')   
            ...
            if cokebaggy ~= nil then
                if DrugList[Player.PlayerData.items[k].name] ~= nil then 
                    if Player.PlayerData.items[k].name == "cokebaggy" and Player.PlayerData.items[k].amount >= 1 then 
                        ...
                        TriggerClientEvent('QBCore:Notify', src, "Bạn không có Coke", 'error')
                        break
                    end
                end
            else
