    ...
    if cards ~= nil and cards[1] ~= nil then
        ...
        for k, v in pairs(Config.ATMModels) do
            ...
            if atm then 
                ...
                QBCore.Functions.Progressbar("accessing_atm", "Truy cập ATM.", 1500, false, true, {
        ...
        QBCore.Functions.Notify("Quý khách chưa có thẻ ATM để truy cập hoặc thanh toán vui lòng đến chi nhánh đặt thẻ.", "error")
    end
end)
...
RegisterNUICallback("removeCard", function(data, cb)
    QBCore.Functions.TriggerCallback('qb-debitcard:server:deleteCard', function(hasDeleted)
        if hasDeleted then
            ...
            QBCore.Functions.Notify('Thẻ đã bị xóa.', 'success')
        else
            QBCore.Functions.Notify('Không thể xóa thẻ.', 'error')
        end
    end, data)
end)