...
AddEventHandler('police:server:TakeOutImpound', function(plate)
    ...
    TriggerClientEvent('QBCore:Notify', src, "Xe được lấy ra khỏi Nhà Giam!")  
end)
...
AddEventHandler('police:server:EscortPlayer', function(playerId)
    ...
    if EscortPlayer ~= nil then
        if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "doctor") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
            ...
            TriggerClientEvent('chatMessage', src, "SYSTEM", "error", "Người chưa chết hoặc bị còng!")
        end
    end
end)
...
AddEventHandler('police:server:BillPlayer', function(playerId, price)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, "Bạn đã nhận được một khoản tiền phạt $"..price)
        end
    end
end)
...
AddEventHandler('police:server:JailPlayer', function(playerId, time)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('QBCore:Notify', src, "Bạn đã đưa người đó vào tù "..time.." tháng")
        end
    end
end)
...
AddEventHandler('police:server:SearchPlayer', function(playerId)
    ...
    if SearchedPlayer ~= nil then 
        ...
        TriggerClientEvent('QBCore:Notify', SearchedPlayer.PlayerData.source, "Bạn đang được tìm kiếm..")
    end
end)
...
AddEventHandler('police:server:SeizeCash', function(playerId)
    ...
    if SearchedPlayer ~= nil then 
        ...
        TriggerClientEvent('QBCore:Notify', SearchedPlayer.PlayerData.source, "Tiền mặt của bạn bị tịch thu..")
    end
end)
...
AddEventHandler('police:server:SeizeDriverLicense', function(playerId)
    ...
    if SearchedPlayer ~= nil then
        ...
        if driverLicense then
            ...
            TriggerClientEvent('QBCore:Notify', SearchedPlayer.PlayerData.source, "Giấy phép lái xe của bạn đã bị tịch thu..")
        else
            TriggerClientEvent('QBCore:Notify', src, "Không thể tịch thu giấy phép lái xe..", "error")
        end
    end
end)
...
AddEventHandler('police:server:RobPlayer', function(playerId)
    ...
    if SearchedPlayer ~= nil then 
        ...
        TriggerClientEvent('QBCore:Notify', SearchedPlayer.PlayerData.source, "Bạn đã bị cướp $"..money.."..")
    end
end)
...
AddEventHandler('police:server:Impound', function(plate, fullImpound, price)
    local src = source
    local price = price ~= nil and price or 100
    if IsVehicleOwned(plate) then
        if not fullImpound then
            ...
            TriggerClientEvent('QBCore:Notify', src, "Xe được đưa vào kho $"..price.."!")
        else
            exports['ghmattimysql']:execute('UPDATE player_vehicles SET state = @state WHERE plate = @plate', {['@state'] = 2, ['@plate'] = plate})
            TriggerClientEvent('QBCore:Notify', src, "Xe bị tịch thu hoàn toàn!")
        end
    end
end)
...
AddEventHandler('evidence:server:AddBlooddropToInventory', function(bloodId, bloodInfo)
    ...
    if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Bạn phải có một túi bằng chứng trống", "error")
    end
end)
...
AddEventHandler('police:server:SetTracker', function(targetId)
    ...
    if TrackerMeta then
        ...
        TriggerClientEvent('QBCore:Notify', targetId, 'Vòng chân của bạn đã được tháo ra.', 'error', 5000)
        ...
        TriggerClientEvent('QBCore:Notify', targetId, 'Bạn đeo một dây đeo mắt cá chân.', 'error', 5000)
        TriggerClientEvent('QBCore:Notify', source, 'Bạn đeo một dây đeo mắt cá chân để '..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname, 'error', 5000)
...
AddEventHandler('police:server:SendTrackerLocation', function(coords, requestId)
    ...
    local msg = "Vị trí của "..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname.." được đánh dấu trên bản đồ của bạn."
...
QBCore.Commands.Add("pobject", "Place/Delete An Object (Police Only)", {{name="type", help="Type object you want or 'delete' to delete"}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "police" then
        ...
        TriggerClientEvent('QBCore:Notify', source, 'Chỉ dành cho dịch vụ khẩn cấp', 'error')
    end
end)
...
QBCore.Commands.Add("palert", "Make a police alert", {{name="alert", help="The Police alert"}}, false, function(source, args)
    ...
    if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
        if args[1] ~= nil then
            ...
            TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Bạn phải nhập tin nhắn!")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Chỉ dành cho dịch vụ khẩn cấp', 'error')
    end
...
QBCore.Commands.Add("jail", "Jail Player (Police Only)", {{name="id", help="Player ID"},{name="time", help="Time they have to be in jail"}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "police" then
        ...
        if time > 0 then
            ...
            TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Thời gian phải cao hơn 0")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Chỉ dành cho dịch vụ khẩn cấp', 'error')
    end
    ...
    if Player.PlayerData.job.name == "police" then
        if Plates ~= nil and Plates[args[1]:upper()] ~= nil then
            if Plates[args[1]:upper()].isflagged then
                ...
                TriggerClientEvent('chatMessage', source, "REPORTING ROOM", "error", "Xe không được gắn cờ!")
            end
        else
            TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Xe không được gắn cờ!")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Chỉ dành cho dịch vụ khẩn cấp', 'error')
    end
    ...
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" then
        ...
        if OtherPlayer ~= nil then
            if OtherPlayer.PlayerData.job.name == "tow" then
                ...
                TriggerClientEvent('chatMessage', OtherPlayer.PlayerData.source, "SYSTEM", "warning", "Bạn đã nhận được $ 500 cho dịch vụ của mình!")
...
QBCore.Commands.Add("911", "Send a report to emergency services", {{name="message", help="Message you want to send"}}, true, function(source, args)
    ...
    if Player.Functions.GetItemByName("phone") ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', source, 'Bạn không có điện thoại', 'error')
    end
end)
...
QBCore.Commands.Add("takedna", "Take a DNA sanple from a person (empty evidence bag needed) (Police Only)", {{"id", "ID of the person"}}, true, function(source, args)
    ...
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) and OtherPlayer ~= nil then
        if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
            ...
            TriggerClientEvent('QBCore:Notify', source, "Bạn phải có một túi bằng chứng trống", "error")
        end
    end
end)
