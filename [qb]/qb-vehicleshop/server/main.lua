...
AddEventHandler('qb-vehicleshop:server:buyVehicle', function(vehicleData, garage)
    ...
    if (balance - vData["price"]) >= 0 then
		...
		TriggerClientEvent("QBCore:Notify", src, "Bạn không có đủ tiền"..format_thousand(vData["price"] - balance), "error", 5000)
    end
end)
...
QBCore.Commands.Add("sellv", "Verkoop voertuig uit Custom Cardealer", {}, false, function(source, args)
...
QBCore.Commands.Add("testdrive", "Testrit maken", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    local TargetId = args[1]
