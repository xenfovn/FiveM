...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("NDH Car Repair")
                ...
                if onDuty then
                    if StashDistance < 20 then
                        ...
                        if StashDistance < 1 then
                            DrawText3Ds(Config.Locations["stash"].x, Config.Locations["stash"].y, Config.Locations["stash"].z, "[E] Mở Kho")
                ...
                if onDuty then
                    if VehicleDistance < 20 then
                        ...
                        if VehicleDistance < 1 then
                            ...
                            if InVehicle then
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, '[E] Cất Xe')
                    ...
                    for k, v in pairs(Config.Plates) do
                        if v.AttachedVehicle == nil then
                                ...
                                if PlateDistance < 2 then
                                    ...
                                    if IsPedInAnyVehicle(PlayerPedId()) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z + 0.3, "[E] Kiểm Tra Phương Tiện")
                                            if IsControlJustPressed(0, 38) then
                                                DoScreenFadeOut(150)
                                                Wait(150)
                                            ...
                                            QBCore.Functions.Notify("Xe Đạp Không Thể Sửa Chữa!", "error")
                                        end
                                    end
                                end
                            end
                            ...
                            if PlateDistance < 3 then
                                inRange = true
                                DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Mở Menu")
                                if IsControlJustPressed(0, 38) then
                                    OpenMenu()
                                    Menu.hidden = not Menu.hidden
                                end
...
function VehicleOptions()
    ClearMenu()
    Menu.addButton("Kết Thúc Kiểm Tra Xe", "UnattachVehicle", nil)
    -- Menu.addButton("Check Status", "CheckStatus", nil)
    Menu.addButton("Bộ phận xe", "PartsMenu", nil)
    Menu.addButton("Đóng menu", "CloseMenu", nil)
end
...
function PartsMenu()
    ...
    Menu.addButton("Trở lại", "VehicleOptions", nil) 
    Menu.addButton("Đóng menu", "CloseMenu", nil) 
end
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Sửa Chữa ("..QBCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("Trở lại", "VehicleOptions", nil)
    Menu.addButton("Đóng menu", "CloseMenu", nil) 
end
            ...
            if v.name == PartData.item then
                if v.amount >= PartData.costs then
                    QBCore.Functions.Progressbar("repair_part", Config.ValuesLabels[part].." repairing", math.random(5000, 10000), false, true, {
                        ...
                        QBCore.Functions.Notify("Sửa chữa bị hủy bỏ", "error")
                    end)
                    break
                else
                    QBCore.Functions.Notify('Không có đủ vật liệu trong kho', 'error')
                end
                break
            else
                QBCore.Functions.Notify('Không có đủ vật liệu trong kho', 'error')
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        QBCore.Functions.Notify("Trạng thái chưa biết", "error")
                    end
                else
                    QBCore.Functions.Notify("Không phải là một chiếc xe hợp lệ", "error")
                end
            else
                QBCore.Functions.Notify("Bạn không đủ gần với xe", "error")
            end
        else
            QBCore.Functions.Notify("Bạn phải ở trong xe trước", "error")
        end
    else
        QBCore.Functions.Notify("Bạn phải ở ngoài xe", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            ...
            QBCore.Functions.Notify("Bạn không phải là người lái xe hoặc trên một chiếc xe đạp", "error")
        end
    else
        QBCore.Functions.Notify("Bạn không ở trong một chiếc xe", "error")
    end
end)
                ...
                if #(pos - vehpos) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            QBCore.Functions.Progressbar("repair_advanced", "Sửa chữa xe", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                openingDoor = false
                                ClearPedTasks(PlayerPedId())
                                QBCore.Functions.Notify("Quá trình bị hủy", "error")
                            end)
                        else
                            QBCore.Functions.Notify("Không phải là một phần hợp lệ", "error")
                        end
                    else
                        QBCore.Functions.Notify("Không phải là một chiếc xe hợp lệ", "error")
                    end
