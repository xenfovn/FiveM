...
QBCore.Commands.Add("setvehiclestatus", "Đặt trạng thái xe", {{name="part", help="Nhập phần bạn muốn chỉnh sửa"}, {name="amount", help="Tỷ lệ phần trăm cố định"}}, true, function(source, args)
...
AddEventHandler('qb-vehicletuning:server:CheckForItems', function(part)
    ...
    if RepairPart ~= nil then
        if RepairPart.amount >= Config.RepairCostAmount[part].costs then
            ...
            TriggerClientEvent('QBCore:Notify', src, "Bạn không có đủ "..QBCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." (min. "..Config.RepairCostAmount[part].costs.."x)", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "Bạn không có "..QBCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." bij je!", "error")
    end
end)
...
QBCore.Commands.Add("setmechanic", "Cho ai đó công việc cơ khí", {{name="id", help="ID Of The Player"}}, false, function(source, args)
    ...
    if IsAuthorized(Player.PlayerData.citizenid) then
        ...
        if TargetId ~= nil then
            ...
            if TargetData ~= nil then
                ...
                TriggerClientEvent('QBCore:Notify', TargetData.PlayerData.source, "Bạn đã được thuê làm nhân viên Sửa Xe!")
                TriggerClientEvent('QBCore:Notify', source, "Bạn có ("..TargetData.PlayerData.charinfo.firstname..") Được thuê dưới dạng nhân viên Sửa Xe!")
            end
        else
            TriggerClientEvent('QBCore:Notify', source, "Bạn phải cung cấp ID người chơi!")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, "Bạn không thể làm điều này!", "error") 
    end
end)

QBCore.Commands.Add("firemechanic", "Bắn một thợ máy.", {{name="id", help="ID của người chơi"}}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
        ...
        if TargetId ~= nil then
            ...
            if TargetData ~= nil then
                if TargetData.PlayerData.job.name == "mechanic" then
                    ...
                    TriggerClientEvent('QBCore:Notify', TargetData.PlayerData.source, "Bạn đã bị sa thải khỏi nhân viên Sửa Xe!")
                    TriggerClientEvent('QBCore:Notify', source, "Bạn có ("..TargetData.PlayerData.charinfo.firstname..") Bị bắn như nhân viên sửa xe!")
                else
                    TriggerClientEvent('QBCore:Notify', source, "Bạn không phải là nhân viên của AutoCare!", "error")
                end
            end
        else
            TriggerClientEvent('QBCore:Notify', source, "Bạn phải cung cấp ID người chơi!", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, "Bạn không thể làm điều này!", "error")
    end
