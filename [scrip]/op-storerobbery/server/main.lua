...
RegisterServerEvent('op-storerobbery:server:takeMoney')
AddEventHandler('op-storerobbery:server:takeMoney', function(register, isDone)
    local src   = source
...
RegisterServerEvent('op-storerobbery:server:setRegisterStatus')
AddEventHandler('op-storerobbery:server:setRegisterStatus', function(register)
    TriggerClientEvent('op-storerobbery:client:setRegisterStatus', -1, register, true)
    Config.Registers[register].robbed   = true
    Config.Registers[register].time     = Config.resetTime
end)

RegisterServerEvent('op-storerobbery:server:setSafeStatus')
AddEventHandler('op-storerobbery:server:setSafeStatus', function(safe)
    TriggerClientEvent('op-storerobbery:client:setSafeStatus', -1, safe, true)
    Config.Safes[safe].robbed = true

    SetTimeout(math.random(40, 80) * (60 * 1000), function()
        TriggerClientEvent('op-storerobbery:client:setSafeStatus', -1, safe, false)
        Config.Safes[safe].robbed = false
    end)
end)

RegisterServerEvent('op-storerobbery:server:SafeReward')
AddEventHandler('op-storerobbery:server:SafeReward', function(safe)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('op-storerobbery:server:callCops')
AddEventHandler('op-storerobbery:server:callCops', function(type, safe, streetLabel, coords)
    local cameraId = 4
    if type == "safe" then
    ...
    local alertData = {
        ...
        description = "Someone is trying to rob a store "..streetLabel.." (CAMERA ID: "..cameraId..")"
    }
    TriggerClientEvent("op-storerobbery:client:robberyCall", -1, type, safe, streetLabel, coords)
    TriggerClientEvent("op-phone:client:addPoliceAlert", -1, alertData)
end)
    ...
    while true do
        for k, v in pairs(Config.Registers) do
            if Config.Registers[k].time > 0 and (Config.Registers[k].time - Config.tickInterval) >= 0 then
                ...
                TriggerClientEvent('op-storerobbery:client:setRegisterStatus', -1, k, false)
            end
        end
...
QBCore.Functions.CreateCallback('op-storerobbery:server:isCombinationRight', function(source, cb, safe)
    cb(SafeCodes[safe])
end)

QBCore.Functions.CreateCallback('op-storerobbery:server:getPadlockCombination', function(source, cb, safe)
    cb(SafeCodes[safe])
end)

QBCore.Functions.CreateCallback('op-storerobbery:server:getRegisterStatus', function(source, cb)
    cb(Config.Registers)
end)

QBCore.Functions.CreateCallback('op-storerobbery:server:getSafeStatus', function(source, cb)
    cb(Config.Safes)
end)