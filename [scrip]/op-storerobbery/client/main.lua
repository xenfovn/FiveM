                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if math.random(100) <= 50 then
                                        TriggerServerEvent('op-hud:Server:GainStress', math.random(1, 3))
                                    end
                                    if Config.Safes[safe].type == "keypad" then
                                        ...
                                        QBCore.Functions.TriggerCallback('op-storerobbery:server:getPadlockCombination', function(combination)
                                    ...
                                    if not copsCalled then
                                        ...
                                        TriggerServerEvent("op-storerobbery:server:callCops", "safe", currentSafe, streetLabel, pos)
                                        copsCalled = true
                                    end
                                else
            ...
            if CurrentCops >= Config.MinimumStoreRobberyPolice then
                if usingAdvanced then
                    ...
                    if not copsCalled then
                        ...
                        print('1')
                        TriggerServerEvent("op-storerobbery:server:callCops", "cashier", currentRegister, streetLabel, pos)
                        copsCalled = true
                    end
                else
...
function setupRegister()
    QBCore.Functions.TriggerCallback('op-storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    QBCore.Functions.TriggerCallback('op-storerobbery:server:getSafeStatus', function(Safes)
...
RegisterNUICallback('success', function()
    if currentRegister ~= 0 then
        lockpick(false)
        TriggerServerEvent('op-storerobbery:server:setRegisterStatus', currentRegister)
        local lockpickTime = 300000
        ...
        QBCore.Functions.Progressbar("search_register", "Emptying register..", lockpickTime, false, true, {
            ...
            TriggerServerEvent('op-storerobbery:server:takeMoney', currentRegister, true)            
            currentRegister = 0
        end, function() -- Cancel
            openingDoor = false
            ClearPedTasks(GetPlayerPed(-1))
...
function LockpickDoorAnim(time)
    ...
    Citizen.CreateThread(function()
        while openingDoor do
            ...
            Citizen.Wait(2000)
            time = time - 2
            TriggerServerEvent('op-storerobbery:server:takeMoney', currentRegister, false)
            if time <= 0 then
                openingDoor = false
        ...
        if won then
            if currentSafe ~= 0 then
                if not Config.Safes[currentSafe].robbed then
                    ...
                    TriggerServerEvent("op-storerobbery:server:SafeReward", currentSafe)
                    TriggerServerEvent("op-storerobbery:server:setSafeStatus", currentSafe)
                    currentSafe = 0
                    takeAnim()
                end
            else
...
RegisterNUICallback('TryCombination', function(data, cb)
    QBCore.Functions.TriggerCallback('op-storerobbery:server:isCombinationRight', function(combination)
...
RegisterNetEvent('op-storerobbery:client:setRegisterStatus')
AddEventHandler('op-storerobbery:client:setRegisterStatus', function(register, bool)
    Config.Registers[register].robbed = bool
end)

RegisterNetEvent('op-storerobbery:client:setSafeStatus')
AddEventHandler('op-storerobbery:client:setSafeStatus', function(safe, bool)
    Config.Safes[safe].robbed = bool
end)

RegisterNetEvent('op-storerobbery:client:robberyCall')
AddEventHandler('op-storerobbery:client:robberyCall', function(type, key, streetLabel, coords)
    if PlayerJob.name == "police" and onDuty then
        ...
        TriggerEvent('op-policealerts:client:AddPoliceAlert', {
            timeOut = 5000,
